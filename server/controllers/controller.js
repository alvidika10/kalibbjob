...
class Controller {
    ...
    static async getJob (req, res, next) {
     ...
     static async getJobById (req, res, next) {
        try {
            const {id} = req.params
            const job = await Job.findByPk(id, { include: [Company, User, Skill]})
            res.status(200).json(job)
        } catch (error) {
            next(error)
        }
    ...
    static async adminAddJob(req, res, next) {
        ...
        try {
            console.log("Add")
            let {title, description, companyId, jobType, skills} = req.body
            console.log(req.body)
            const job = await Job.create({title, description, companyId, authorId:req.user.id, jobType}, {transaction: t})
    ...
    static async adminEditJob(req, res, next) {
        try {
            ...
            const updateJob = await findJob.update({title, description, companyId, authorId: req.user.id, jobType})
